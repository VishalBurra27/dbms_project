        ...
        _tpl: [
            '<div class="zh-add-question-form" style="margin:0;">',
                '<div class="zg-section-big clearfix">',
                    '<div class="add-question-section-title">send out：</div>',
                    '<div class="zg-form-text-input add-question-title-form" style="position: relative;">',
                        '<input type="text" class="js-name zg-editor-input zu-seamless-input-origin-element" placeholder="full name" style="height:22px;min-height:auto;"></textarea>',
                    '</div>',
                '</div>',
                '<div class="zg-section-big">',
                    '<div class="add-question-section-title">content：</div>',
                        ...
                        '<div class="zm-editable-editor-wrap no-toolbar">',
                            '<div class="zm-editable-editor-outer">',
                                '<div class="zm-editable-editor-field-wrap">',
                                    '<textarea class="js-content zm-editable-editor-field-element editable" placeholder="private message content" style="font-style:italic;width:98%;"></textarea>',
                                '</div>',
                            '</div>',
                        '</div>',
                    '</div>',
    ...
    function fStaticShow(oConf) {
        ...
        var oPopup = new Popup({
            ...
            ok: function () {
                ...
                var oData = oAdd.val();
                if (!oData.toName) {
                    that.error('Please fill in your name');
                    return true;
                }
                if (!oData.content) {
                    that.error('Please fill in the contents of the private letter');
                    return true;
                }
                // 避免重复提交
                if (bSubmit) {
                ...
                $.ajax({
                    ...
                    if (oResult.code === 999) {
                        ...
                        that.error(oResult.msg || 'An error occurred. Please try again');
                    } else {
