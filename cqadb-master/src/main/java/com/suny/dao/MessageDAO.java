...
/**
 * Created by admin on 23-2-4.5:21 pm
 */
@Mapper
public interface MessageDAO {
    ...
    @Select({"select ", INSERT_FIELDS, " , id as id from ( select * from ", TABLE_NAME, " where from_id=#{userId} or to_id=#{userId} order by create_date desc) tt order by create_date desc limit #{offset}, #{limit}"})
